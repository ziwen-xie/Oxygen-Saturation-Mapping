...
def filter1nd(y,count):
    ...
    b,a = signal.butter(2,2,fs=60)
    z = signal.filtfilt(b,a,y)
    return z

def plot3():
    im = cv2.imread("nirhand.jpg")
    r = cv2.selectROI(im)
    print(r)
    cv2.destroyAllWindows()
        
    nir_black,count,t1 = process("nirblack.h264",r)
    #nir_black = filter1nd(nir_black,count)
    nir_hand,count,t2 = process("nirhand.h264",r)
    #nir_hand = filter1nd(nir_hand,count)
    nir_white,count,t3 = process("nirwhite.h264",r)
